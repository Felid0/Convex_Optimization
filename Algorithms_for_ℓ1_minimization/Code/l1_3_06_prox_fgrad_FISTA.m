function [x,out]= l1_3_06_prox_fgrad_FISTA(x0, A, b, mu, opts)
%% Readme:
%   This fucntion is defined to solve the problem below 
%   by proximal gradient descent method using FISTA.
% 
...
%% Step 1: Initialization & reformulation
    ...
    % set proximal operator w.r.t. h(x) = mu ||x||_1
    prox_l1 = @(x,mu) sign(x).*max(0, abs(x) - mu);    

%% Step 2: Fast-Proximal-Gradient-Descent
    ...
    while mu_finder > mu 
        ...
        for step = 1:maxsteps
            ...
            g_grad_y = Q * y + c;
            z = y - ss * g_grad_y;
            x = prox_l1(z,ss * mu_finder);     % Proximal mapping with line search
            
            % step2.3: update objective value path at mu 
            i = i + 1;
%             objval_path(i) = 0.5 * x'* Q *x + c'*x + con + mu * norm(x, 1);              
